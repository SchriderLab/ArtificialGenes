...
for e in range(epochs):
    ...
    if e%save_that == 0 or e == epochs:
        ...
        generated_genomes_df.to_csv(os.path.join(odir, str(e)+"_output.hapt"), sep=" ", header=False, index=False)
    
        #Output lossess
        pd.DataFrame(losses).to_csv(os.path.join(odir, str(e)+"_losses.txt"), sep=" ", header=False, index=False)
        fig, ax = plt.subplots()
        ...
        for pop, color in zip(pops,colors):
            ...
            ax.scatter(PCs_df.loc[ix, 'PC1']
                       ...
                       , s = 50, alpha=0.2)
        ax.legend(pops)
        fig.savefig(os.path.join(odir, str(e)+'_pca.pdf'), format='pdf')
