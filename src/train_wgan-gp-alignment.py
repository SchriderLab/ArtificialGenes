...
def parse_args():
    ...
    parser.add_argument("--num_in", default="2000", help="number of input alignments")
    parser.add_argument("--epochs", default="10000")
    parser.add_argument("--ag_size", default="64", help="number of artificial genomes (haplotypes) to be created"
    ...
    parser.add_argument("--critic_iter", default="5")  ### Changed default from 5 to 2 (9-21-2021)
...
def main():
    ...
    print(idir+"*.csv")
    files = np.random.choice(glob.glob(idir+"*.csv"),num_in)
    ...
    save_image(data[torch.randperm(num_in)[:64]].float(), os.path.join(odir, "input.png"))
    ...
    # Make generator
    generator = DC_Generator(data_size, latent_size, negative_slope).to(device)
    generator.apply(weights_init)
    # Make discriminator
    discriminator = DC_Discriminator(data_size, negative_slope).to(device)
    ...
    for i in range(epochs):
        ...
        for j, X_real in enumerate(dataloader):
            ...
            for h in range(critic_iter):
                ...
                # calculate gradient penalty (losses accumulate, so in step here we have d_loss_fake - d_loss_real + grad_penalty)
            ...
            if j % epoch_length - 1 == 0 and j != 0:
                losses.append((wasserstein_d.item(), gen_loss.item(), d_loss_real.item(), d_loss_fake.item(), g_penalty.item()))
                logging.info("Epoch:\t%d/%d Discriminator loss: %6.4f Generator loss: %6.4f Crit_real: %6.4f Crit_fake: %6.4f Grad_Pen: %6.4f" % (i + 1, epochs, wasserstein_d.item(), gen_loss.item(), d_loss_real.item(), d_loss_fake.item(), g_penalty.item()))
        ...
        if save_freq != 0 and (i % save_freq == 1 or i == epochs):
            ...
            if ag_size > 0:
                # Create AGs
                generated_genomes_img = create_AGs(generator, i, ag_size, latent_size, data_size, odir, device)
                AG_imgs.append(generated_genomes_img)
                # plot losses and pca
                if args.plot:
