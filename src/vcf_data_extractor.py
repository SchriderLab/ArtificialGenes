# for 1000G vcf files in the longleaf /datacommons directory

import pandas as pd
import os
import csv

dir_path = "/datacommons/1000genomes"
key_file = "integrated_call_samples_v3.20130502.ALL.panel"
genome_file = "ALL.chr14.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf"
key_path = os.path.join(dir_path, key_file)
ifile = os.path.join(dir_path, genome_file)

key_dataframe = pd.read_csv(key_path, delimiter="\t")
key_dataframe.drop(columns=['Unnamed: 4', "Unnamed: 5"], inplace=True)
key_dataframe = key_dataframe[key_dataframe['pop'] == 'YRI']
...
for sequence in data.keys():
    ...
    for i, column in enumerate(data_temp.columns):
        ...
        sequence_df_one["YRI_{}".format(i)] = temp[0]
        sequence_df_two["YRI_{}".format(i)] = temp[1]
...
full_dataframe_one.to_csv("./full_dataframe_one.csv",index=False)
full_dataframe_two.to_csv("./full_dataframe_two.csv", index=False)
